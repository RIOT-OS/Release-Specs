...
def test_command_exists(monkeypatch):
    monkeypatch.setattr(
        testutils.native.subprocess, "check_call", lambda *args, **kwargs: None
    )
...
def test_ip_addr_del(monkeypatch):
    monkeypatch.setattr(
        testutils.native.subprocess, "run", lambda *args, **kwargs: None
    )
    testutils.native.ip_addr_del("test", "2001:db8::1")


@pytest.mark.parametrize("args", ((), ("fe80::1",)))
...
def test_bridged(monkeypatch, expected):
    monkeypatch.setattr(
        testutils.native, "_check_bridged", lambda *args, **kwargs: expected
    )
...
95: tap0: ...
    ...
    assert testutils.native.get_link_local("tap0") == "fe80::e0bc:7dff:fecb:f54f"
...
@pytest.mark.parametrize("ping_cmd,expected", [("ping6", "ping6"), ("ping", "ping -6")])
def test_get_ping_cmd(monkeypatch, ping_cmd, expected):
    monkeypatch.setattr(testutils.native, "command_exists", lambda cmd: cmd == ping_cmd)
    assert testutils.native.get_ping_cmd() == expected


def test_get_ping_cmd_no_ping(monkeypatch):
    monkeypatch.setattr(testutils.native, "command_exists", lambda cmd: False)
